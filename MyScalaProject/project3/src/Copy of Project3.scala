  ...
  class Watcher(noOfNodes: Int, noOfRequests: Int) extends Actor {
    ...
    final def receive = {
      ...
      case SendLiveNeighbor =>
        // We are assuming that the nodesArr Array is always kept sorted.
        var length = nodesArr.length
        var index = sender.path.name.drop(6).toInt
        if (length == 0) {
          sender ! Pastry.RecieveLiveNeighbor(null)
        } else {
          var closestNeighbor = nodesArr.minBy(a => (a.nodeRef.path.name.drop(6).toInt - index).abs)
          sender ! Pastry.RecieveLiveNeighbor(closestNeighbor.nodeRef)
        }
  ...
  class Pastry(b: Int) extends Actor {
    ...
    def route(msg: String, key: Node, hopCount: Int = 0): Boolean = {
      ...
      if (tmpArr.length > 0 && key.nodeId >= tmpArr.head.nodeId && key.nodeId <= tmpArr.last.nodeId) {
        ...
        if (routingEntry != null) {
          ...
          ctr3 = 0
          while (ctr3 < tmpArr.length && !found) {
            ...
            if (prefixSize >= currPrefixSize) {
              ...
              if (nodeDiff < currNodeIdDiff) {
                handler.forward(msg, key, tmpArr(ctr3)) // call to application.
                tmpArr(ctr3).nodeRef ! FinalHopMsg(msg, key, hopCount)
                found = true
                println("Self: " + selfNode.nodeId + " Routing " + key.nodeId + " to Rare " + tmpArr(ctr3).nodeId)
              }
            }
            ctr3 += 1
          } // end of while
    ...
    def sendStatus(key: Node, hop: Int) {
      if (hop == 0) {
        ...
        key.nodeRef ! RecieveStatus(tmpArr ++ Array(selfNode), "neighbor")
      }
      ...
      key.nodeRef ! RecieveStatus(tmpArr ++ Array(selfNode), "routing")
    }
