...
object CopyProject3Bonus {
    ...
    final def receive = LoggingReceive {
      ...
      case AddNewNode(node) =>
        ...
        if (nodesArr.length == noOfNodes) {
          applicationArr.foreach(a => system.scheduler.scheduleOnce(1000 milliseconds, a.pastryRef, Pastry.InitiateMultipleRequests(noOfRequests)))
          ...
          while (fc > 0 && tmpRefArr.length > 0) {
            ...
            system.scheduler.scheduleOnce(3000 milliseconds, tmp.pastryRef, Pastry.FAILED)
            tmpRefArr -= tmp
            fc -= 1
          }
        }
  ...
  class Pastry(base: Int, noOfBits: Int) extends Actor {
    ...
    def route(msg: String, key: Node, hopCount: Int = 0): Boolean = {
      ...
      if (l.length > 0) {
        var left = l.minBy(a => a.nodeId)
        if (key.nodeId >= left.nodeId && key.nodeId < selfNode.nodeId) {
          lbool = true
        }
      }
      if (r.length > 0) {
        var right = r.maxBy(a => a.nodeId)
        if (key.nodeId <= right.nodeId && key.nodeId > selfNode.nodeId) {
          rbool = true
        }
      }
      ...
      if (lbool || rbool) {
        val tmp = leafArr.minBy(a => (key.nodeId - a.nodeId).abs)
        ...
        if (routingEntry != null && routingEntry.nodeId != -2 && routingEntry.nodeId != -1) {
    ...
    private def removeNodeFromTables(ref: ActorRef) {
      var index = leafArr.indexWhere(a => a.nodeRef == ref)
      if (index != -1) {
        var id = leafArr(index).nodeId
        leafArr = leafArr.filter(a => a.nodeId != id)
        var l = leafArr.filter(a => a.nodeId < selfNode.nodeId)
        var r = leafArr.filter(a => a.nodeId > selfNode.nodeId)
      ...
      if (index != -1) {
        var id = neighborArr(index).nodeId
        neighborArr = neighborArr.filter(a => a.nodeId != id)
      ...
      while (ctr < routingArr.length) {
        ...
        while (col < routingArr(ctr).length) {
          if (routingArr(ctr)(col) != null && routingArr(ctr)(col).nodeRef == ref && routingArr(ctr)(col).nodeId != -2 && routingArr(ctr)(col).nodeId != -1) {
            //println("DEBUG " + selfNode.nodeId + "  " + routingArr(ctr)(col).nodeId + "    " + ctr + "    " + col)
            routingArr(ctr)(col).nodeId = -2
            var tmp = routingArr(ctr).filter(a => a != null && a.nodeId != -1 && a.nodeId != -2)
            tmp.foreach(a => a.nodeRef ! RequestingTable("routing", ctr, col, ctr))
          }
          col += 1
        }
        ctr += 1
    ...
    def Alive: Receive = LoggingReceive {
      ...
      case FAILED =>
        ...
        parent ! Watcher.Terminate(selfNode)
        println("Node Dead " + selfNode.nodeId + "   " + selfNode.nodeRef.path.name)
        become(Dead)
    }
